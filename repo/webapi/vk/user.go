...
func (r *UserWebAPI) User(userID int) (entity.User, error) {
	id := strconv.Itoa(userID)
	apiUrl := fmt.Sprintf("https://api.vk.com/method/users.get?user_ids=%s&access_token=%s&v=5.131", id, r.AccessToken)
	...
	if len(ur.Response) == 0 {
		return entity.User{}, fmt.Errorf("err: user not found")
	}
...
func (r *UserWebAPI) FriendsByID(userID int) ([]entity.User, error) {
	id := strconv.Itoa(userID)
	apiUrl := fmt.Sprintf("https://api.vk.com/method/friends.get?user_id=%s&fields=counters&access_token=%s&v=5.131", id, r.AccessToken)
	...
	if fr.Error.Code != 0 {
		if fr.Error.Code == 100 {
			return nil, fmt.Errorf("err: invalid user_id")
		} else if fr.Error.Code == 30 {
			return nil, fmt.Errorf("err: user is private")
		} else {
